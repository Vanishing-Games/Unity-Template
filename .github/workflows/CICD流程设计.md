 ...
 * @LastEditTime: 2025-08-16 16:42:17
 * // -----------------------------------------------------------------------------
-->
<!--
 * // -----------------------------------------------------------------------------
...
| `[SKIP FORMAT]` | 跳过代码格式化检查步骤(如果有) | 仅更新文档或配置时 |
| `[SKIP TEST]` | 跳过单元测试步骤(如果有) | 仅更新文档或配置时 |
| `[SKIP BUILD]` | 跳过构建步骤(如果有) | 在功能分支测试构建时 |
| `[ADD TEST]` | 增加单元测试步骤(如果没有) | 希望进行单元测试时 |
| `[ADD BUILD]` | 增加构建步骤(如果没有) | 希望进行构建测试时 |
| `[ADD DEPLOY]` | 增加部署步骤(如果没有) | 希望部署构建产物时,此时构建产物命名是{项目名称}_{PR名称} |
