...
namespace UnityEngine.Rendering.Universal
{
    internal static class RendererLighting
    {
        ...
        private static readonly int k_BlendOpID = Shader.PropertyToID("_BlendOp");
        ...
        {
            SetBlendModes(material, src, dst, BlendOp.Add);
        }

        private static void SetBlendModes(Material material, BlendMode src, BlendMode dst, BlendOp op)
        {
            material.SetFloat(k_SrcBlendID, (float)src);
            material.SetFloat(k_DstBlendID, (float)dst);
            material.SetFloat(k_BlendOpID, (float)op);
        }
        ...
        private static uint GetLightMaterialIndex(Light2D light, bool isVolume, bool useShadows)
        {
            ...
            bitIndex++;
            var overlapBits = ((uint)light.overlapOperation & 0x3u) << bitIndex;
            bitIndex += 2;
            ...
            return fastQualityBit | pointCookieBit | overlapBits | shapeBit | volumeBit | useNormalMap | useShadowMap;
        }
        ...
        private static Material CreateLightMaterial(Renderer2DData rendererData, Light2D light, bool isVolume, bool useShadows)
        {
            ...
            if (!isVolume)
            {
                switch (light.overlapOperation)
                {
                    case Light2D.OverlapOperation.Additive:
                        SetBlendModes(material, BlendMode.One, BlendMode.One, BlendOp.Add);
                        material.EnableKeyword(k_UseAdditiveBlendingKeyword);
                        break;
                    case Light2D.OverlapOperation.UseMax:
                        SetBlendModes(material, BlendMode.One, BlendMode.One, BlendOp.Max);
                        material.EnableKeyword(k_UseAdditiveBlendingKeyword);
                        break;
                    case Light2D.OverlapOperation.AlphaBlend:
                    default:
                        SetBlendModes(material, BlendMode.SrcAlpha, BlendMode.OneMinusSrcAlpha, BlendOp.Add);
                        break;
                }
            }
            else
