...
{
    [MenuItem("Tools/OdinEditorSample/🛠️ 重新生成菜单")]
    public static void GenerateMenus()
    {
        var allMethods = AppDomain
            .CurrentDomain.GetAssemblies()
            .Where(asm => !asm.FullName.StartsWith("Unity") && !asm.FullName.StartsWith("System")) // 加速过滤
            .SelectMany(a =>
            {
                try
                {
        ...
        if (!allMethods.Any())
        {
            Debug.LogWarning("⚠️ 未找到任何带 [AutoOdinWindowMenu] 的静态方法。");
            return;
        }
        ...
        Debug.Log($"✅ 已生成自动菜单代码，共 {allMethods.Count} 个项。位置：{filePath}");
    }
...
{{ for method in methods }}
    ...
    public static void {{ method.unique_function_name }}()
    {
        ...
        if (method == null)
        {
            Debug.LogError(""未能找到方法 {{ method.type_full_name }}.{{ method.method_name }}"");
            return;
        }
