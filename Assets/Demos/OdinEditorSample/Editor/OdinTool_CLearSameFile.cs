...
public class OneKeySearchDuplicateFiles : SerializedScriptableObject
{
    ...
    private Dictionary<string, List<string>> sameMD5Group = new();

    [ShowInInspector]
    [DictionaryDrawerSettings(KeyLabel = "文件名称", ValueLabel = "绝对路径列表")]
    private Dictionary<string, List<string>> sameNameGroup = new();
    ...
    private Dictionary<string, List<string>> sameMD5Result5Group = new();
    ...
    private Dictionary<string, List<string>> sameNameResultGroup = new();
    ...
    public void StartSearch()
    {
        ...
        ResetData();
        DirectoryInfo directoryInfo = new(targetSearchFolder);
        var filesGroup = directoryInfo
    ...
    {
        sameMD5Result5Group = sameMD5Group
            .Where(x => x.Value.Count > 1)
            .ToDictionary(p => p.Key, p => p.Value);
        sameNameResultGroup = sameNameGroup
            .Where(x => x.Value.Count > 1)
            .ToDictionary(p => p.Key, p => p.Value);
    }
    ...
    [ProgressBar(
        ...
        "maxCount",
        DrawValueLabel = true,
        ValueLabelAlignment = TextAlignment.Left,
        ColorMember = "GetHealthBarColor",
        Height = 30
    )]
    [ShowInInspector]
    [HideLabel]
    ...
    public string GetMD5HashFromFile(string fileFullName)
    {
        try
        {
            FileStream file = new(fileFullName, FileMode.Open);
