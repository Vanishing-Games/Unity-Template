...
namespace Core
{
    ...
    public class StateMachine<TState, TDriver> : IStateMachine<TDriver>
        where TState : struct, IConvertible, IComparable
        where TDriver : class, new()
    {
        ...
        public StateMachine(MonoBehaviour component)
        {
            ...
            enumConverter =
                Delegate.CreateDelegate(typeof(Func<TState, int>), identity.Method)
                as Func<TState, int>;
            ...
            if (enumValues.Length < 1)
            {
                throw new ArgumentException(
                    "Enum provided to Initialize must have at least 1 visible definition"
                );
            }
            ...
            {
                throw new ArgumentException(
                    "Only enums with an underlying type of int are supported"
                );
            }
            ...
            // }
            List<FieldInfo> eventFields = GetFilteredFields(
                typeof(TDriver),
                "MonsterLove.StateMachine.StateEvent"
            );
        ...
        static Dictionary<object, StateMapping<TState, TDriver>> CreateStateLookup(
            StateMachine<TState, TDriver> fsm,
            Array values
        )
        {
            ...
            {
                var mapping = new StateMapping<TState, TDriver>(
                    fsm,
                    (TState)values.GetValue(i),
                    fsm.GetState
                );
        ...
        static TDriver CreateDriver(
            Func<bool> isInvokeAllowedCallback,
            Func<int> getStateIntCallback,
            int capacity,
        ...
        )
        {
            if (fieldInfos == null)
            {
                throw new ArgumentException(
                    string.Format(
                        "Arguments cannot be null. Callback {0} fieldInfos {1}",
                        isInvokeAllowedCallback,
                        fieldInfos
                    )
                );
            }
            ...
            for (int i = 0; i < fieldInfos.Count; i++)
            {
                ...
                ConstructorInfo constructorInfo = fieldInfo.FieldType.GetConstructor(
                    new Type[] { typeof(Func<bool>), typeof(Func<int>), typeof(int) }
                ); //StateEvent(Func<Bool> invokeAllowed, Func<in> getState, int capacity)
                object obj = constructorInfo.Invoke(
                    new object[] { isInvokeAllowedCallback, getStateIntCallback, capacity }
        ...
        static bool ParseName(MethodInfo methodInfo, out TState state, out string eventName)
        {
            ...
            if (
                methodInfo.GetCustomAttributes(typeof(CompilerGeneratedAttribute), true).Length != 0
            )
            {
                return false;
            }
        ...
        static void BindEvents(
            ...
            TState state,
            int stateInt,
            MethodInfo stateTargetDef,
            FieldInfo driverEvtDef
        )
        {
            ...
            catch (ArgumentException)
            {
                throw new ArgumentException(
                    string.Format(
                        "State ({0}_{1}) requires a callback of type: {2}, type found: {3}",
                        state,
                        driverEvtDef.Name,
                        actionType,
                        stateTargetDef
        ...
        static void BindEventsInternal(
            StateMapping<TState, TDriver> targetState,
            Component component,
            MethodInfo method,
            string evtName
        )
            ...
            switch (evtName)
            {
                case "Enter":
                    if (method.ReturnType == typeof(IEnumerator))
                    {
                        ...
                        targetState.EnterRoutine = CreateDelegate<Func<IEnumerator>>(
                            method,
                            component
                        );
                    }
                ...
                case "Exit":
                    if (method.ReturnType == typeof(IEnumerator))
                    {
                        ...
                        targetState.ExitRoutine = CreateDelegate<Func<IEnumerator>>(
                            method,
                            component
                        );
                    }
        ...
        static V CreateDelegate<V>(MethodInfo method, Object target)
            where V : class
        {
            ...
            {
                throw new ArgumentException(
                    "Unable to create delegate for method called " + method.Name
                );
            }
            ...
            switch (genericArgs.Length)
            {
                ...
                default:
                    throw new ArgumentOutOfRangeException(
                        string.Format(
                            "Cannot create Action Type with {0} type arguments",
                            genericArgs.Length
                        )
                    );
            }
        ...
        private IEnumerator ChangeToNewStateRoutine(
            StateMapping<TState, TDriver> newState,
